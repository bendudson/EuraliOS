...
these still only handle =Short= messages:
#+begin_src rust
  impl Message {
      fn to_values(&self)
...
The reply could be an address (=MESSAGE_PCI_ADDRESS=, 257) if it was
found, a not-found message (=MESSAGE_PCI_NOTFOUND=, 258), or something
else:
#+begin_src rust
