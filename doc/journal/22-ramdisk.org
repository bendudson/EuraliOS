...
#+end_src
It first builds the executable with the =test= feature. Unfortunately
the output binary is not named consistently so we use =find= to locate it.
We then strip the binary because if we don't then the binary is 1.9Mb; after
stripping the binary is just 119Kb.
